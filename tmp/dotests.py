...
#>w = aes128_expand_key(b'abcdefghijklmnop')
#>for x in w:
#>    for y in x:
#>        print(f'{y:0>8x} ', end='')
#>print('')
...
def print_state(t):
    print(' '.join(f'{n:0>8x}' for n in t))
...
def aes128_encrypt(key, plaintext):
    ...
    t = [bytes_to_num(plaintext[4*i:4*i + 4]) for i in range(4)]
    print_state(t)
    ...
    for r in range(1, AES_ROUNDS):
        ...
        print('< ', end='')
        print_state([bytes_to_num(t[i]) for i in range(4)])
...
print(aes128_encrypt(key, text).hex())
sys.exit()
